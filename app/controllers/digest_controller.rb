...
class DigestController < ApplicationController
  ...
  def index
    ...
    if @last_tweet_seen
      @tweets_from_api = client.home_timeline(:since_id         => @last_tweet_seen,
                                          :count            => 200,
                                          :include_entities => true)
    else
      @tweets_from_api = client.home_timeline(:count            => 200,
                                          :include_entities => true)
    ...
    @digested_tweets = TwitterDigest::DigestionLogic.digest(@unabridged_tweets)
